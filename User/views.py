...
class Login(APIView):
    ...
    def post(self, request):
        ...
        # Replace the following line with the call to send_otp_on_phone
        send_otp_on_phone(user.phone, user.otp)
...
class SendOTP(APIView):
    def post(self, request):
        ...
        if send_otp_on_phone(mobile_number, otp):
...
#             return Response({'error': 'User with this phone_number does not exist.'}, status=status.HTTP_404_NOT_FOUND)
...
#         return Response({'message': 'OTP has been sent to your phone_number.'}, status=status.HTTP_200_OK)