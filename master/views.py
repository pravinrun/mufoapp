...
class GlobaleTopFanListAll(APIView):
    @method_decorator(authenticate_token)
    def post(self, request, *args, **kwargs):
        serializer = CoinTransferSerializer(data=request.data)
            ...
            try:
                ...
                with transaction.atomic():
                    for profile_model in profiles:
                        receiver_profile = profile_model.objects.filter(uid=receiver.uid)
                        if receiver_profile.exists():
                            recipient = receiver_profile.first()
                            recipient.coins += amount
                            recipient.save()
                            logging.info(f"{amount} $ transferred to {recipient.Name}'s {recipient.usertype}Coins.")
                    ...
                    for profile_model in profiles:
                        ...
                        if sender_profile.exists():
                            ...
                            if sender_user.coins >= amount:
                                ...
                                sender_user.save()
                                GiftTransactionhistory.objects.create(sender=sender, receiver=receiver, amount=amount, created_date=datetime.today())
                                logging.info(f"{amount} $ transferred from {sender.Name}'s {sender.usertype}Coins.")
...
# class Approved_by_admin_Alluser(APIView):
#     def get(self, request):
#         # Filter users approved by the administrator
#         data = Common.objects.filter(approved_by_admin=True)
...
#         user_serializer = UserSerializer(approved_users, many=True)
...
from Audio_Jockey.serializers import UserSerializer  
from Coins_club_owner.serializers import UserSerializer  
from Coins_trader.serializers import UserSerializer  
from Jockey_club_owner.serializers import UserSerializer 
# class Approved_by_admin_Alluser(APIView):
#     def get(self, request):
#         approved_users = Audio_Jockey.objects.filter(Is_Approved=True)
...
# class Approved_by_admin_Alluser(APIView):
    # def get(self, request):
    #     approved_users = Coins_club_owner.objects.filter(Is_Approved=True)
...
# class Approved_by_admin_Alluser(APIView):
#     def get(self, request):
#         approved_users = Coins_trader.objects.filter(Is_Approved=True)
...
# class Approved_by_admin_Alluser(APIView):
#     def get(self, request):
#         approved_users = Jockey_club_owner.objects.filter(Is_Approved=True)
...
#     MODELS = [Audio_Jockey, Coins_club_owner, Coins_trader, Jockey_club_owner]
...
#             approved_users = model.objects.filter(Is_Approved=True)
#             user_serializer = UserSerializer(approved_users, many=True)
...
#         # for model in self.MODELS:
#             # approved_users = model.objects.filter(Is_Approved=True)
#         AudioJockey = request.query_params.get('Audio_Jockey')
#         Jockeyclubowner = request.query_params.get('Jockey_club_owner')
#         Coinsclubowner = request.query_params.get('Coins_club_owner')
#         Coins_trader = request.query_params.get('Coins_trader')
#         if AudioJockey:
#             approved_users = Audio_Jockey.objects.filter(Is_Approved=True)
#             # approved_users = Coins_club_owner.objects.filter(Is_Approved=True)
#             # approved_users = Coins_trader.objects.filter(Is_Approved=True)
...
#     @method_decorator(authenticate_token)
#     def get(self, request):
#         AudioJockey = request.query_params.get('AudioJockey')
#         Jockeyclubowner = request.query_params.get('Jockeyclubowner')
#         Coinstrader = request.query_params.get('Coinstrader')
#         Coinsclubowner = request.query_params.get('Coinsclubowner')

#         response_data = {}

#         if AudioJockey is not None:
#             is_approved_audio = AudioJockey.lower() == 'true'
#             approved_users_audio = Audio_Jockey.objects.filter(Is_Approved=is_approved_audio)
#             user_serializer_audio = UserSerializer(approved_users_audio, many=True)
...
#                     'phone': user_data.get('phone'),
#                 }
#                 for user_data in user_serializer_audio.data
#             ]

#             model_name_audio = Audio_Jockey.__name__
#             response_data[model_name_audio] = data_list_audio
#         if Coinsclubowner is not None:
#             is_approved_owner = Coinsclubowner.lower() == 'true'
#             approved_users_owner = Coins_club_owner.objects.filter(Is_Approved=is_approved_owner)
#             user_serializer_owner = UserSerializer(approved_users_owner, many=True)
...
#                     'phone': user_data.get('phone'),
#                 }
#                 for user_data in user_serializer_owner.data
#             ]

#             model_name_owner =Coins_club_owner .__name__
#             response_data[model_name_owner] = data_list_owner

#         if Coinstrader is not None:
#             is_approved_coinstrader = Coinstrader.lower() == 'true'
#             approved_users_coinstrader = Coins_trader.objects.filter(Is_Approved=is_approved_coinstrader)
#             user_serializer_coinstrader = UserSerializer(approved_users_coinstrader, many=True)

#             data_list_coinstrader = [
#                 {
#                     'id': user_data.get('id'),
#                     'Name': user_data.get('Name'),
#                     'phone': user_data.get('phone'),
#                 }
#                 for user_data in user_serializer_coinstrader.data
#             ]

#             model_name_coinstrader = Coins_trader.__name__
#             response_data[model_name_coinstrader] = data_list_coinstrader

#         if Jockeyclubowner is not None:
#             is_approved_jockey = Jockeyclubowner.lower() == 'true'
#             approved_users_jockey = Jockey_club_owner.objects.filter(Is_Approved=is_approved_jockey)
#             user_serializer_jockey = UserSerializer(approved_users_jockey, many=True)
...
#                     'phone': user_data.get('phone'),
#                 }
#                 for user_data in user_serializer_jockey.data
#             ]

#             model_name_jockey = Jockey_club_owner.__name__
#             response_data[model_name_jockey] = data_list_jockey

#         if AudioJockey is None and Jockeyclubowner is None and Coinstrader is None and Coinsclubowner is None:
...
class Top_fans(APIView):
    ...
    def get(self, request, id=None):
        try:
            ...
            for transaction in sorted_transactions:
                ...
                if from_user_name in total_coins_dict:
                    ...
                    total_coins_dict[from_user_name] = {
                        "Gift_sender": from_user.uid,
                        "user_name": from_user.Name,
                        "profile_picture": from_user.profile_picture if from_user.profile_picture else None,
                        "coins": coins,
                        "uid": from_user.uid,
                        "receiver": recieve
                    }
            ...
            data = sorted(
                [value for value in total_coins_dict.values()],
                key=lambda x: x["coins"],
                reverse=True
            )
